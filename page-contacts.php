            ...
            <div class="tabs__panel">
              <div class="offices__info">
                ...
                <?php if (!empty($office["tel"])) :
                  // Подсчитываем количество ссылок на телефон в тексте
                  $tel_links_count = preg_match_all('/href="tel:/', $office["tel"], $matches);
                  $title = ($tel_links_count > 1)
                    ? esc_html__('Телефоны', 'primasnab')
                    : esc_html__('Телефон', 'primasnab');
                ?>
                  <div class="offices__contacts contacts">
                    <span class="contacts__title"><?php echo $title; ?></span>
                    <div class="contacts__value">
                      <?php echo wp_kses_post(nl2br($office["tel"])); ?>
                    </div>
                  </div>
                <?php endif; ?>
                ...
                <!-- Дополнительная информация (только если не пусто) -->
                <?php if (!empty($office["additional_info"])) : ?>
                  <div class="offices__contacts contacts">
                    <span class="contacts__title"><?php esc_html_e('Дополнительная информация', 'primasnab'); ?></span>
                    <p class="contacts__value"><?php echo wp_kses_post(nl2br($office["additional_info"])); ?></p>
                  </div>
                <?php endif; ?>
